...
},cleanup:function(){},resetsEvents:function(){return false
...
},setVideoAssetCurrentTimePosition:function(a){this.controller.callLayerFunction("freewheel","setVideoAssetCurrentTimePosition",[a])
...
},cleanup:function(){this.proxy._ve.getPlaybackElement().style.cssText=this.cleanCSS
},onAdManagerLoaded:function(){if(typeof tv=="undefined"||typeof tv.freewheel=="undefined"||typeof tv.freewheel.SDK=="undefined"||typeof tv.freewheel.SDK.AdManager=="undefined"){this.ready=false
}else{this._adManager=new tv.freewheel.SDK.AdManager();
this.ready=true
}this._callback.call()
...
},setAdPlayState:function(a){if(a){this.slotManager.slot.pause()
}else{this.slotManager.slot.resume()
}},setVideoState:function(a){this._context.setVideoState(a)
},setVideoAssetCurrentTimePosition:function(a){this._context.setVideoAssetCurrentTimePosition(a)
},getVideoDisplaySize:function(){return this._context.getVideoDisplaySize()
},setVideoDisplaySize:function(e,d,c,b,a){if(this._context){this._context.setVideoDisplaySize(e,d,c,b,a)
...
},skipCurrentAd:function(){this.controller.callLayerFunction("freewheel","skipCurrentAdForSlot",[this.index])
}});
$pdk.ns("$pdk.plugin.freewheel.noop");
$pdk.plugin.freewheel.noop.PdkNoopPlayer=$pdk.extend(function(){},(function(){return{_controller:null,_volumeSetCallback:null,_muteSetCallback:null,_pauseSetCallback:null,_onPlayerPauseListener:null,_onPlayerUnpauseListener:null,_onVolumeChangeListener:null,_onMuteListener:null,_active:false,constructor:function(a,b,d,e){this._controller=a;
this._volumeSetCallback=b;
this._muteSetCallback=d;
this._pauseSetCallback=e;
var c=this;
this._onPlayerUnpauseListener=function(f){c._onPlayerUnpause.apply(c,arguments)
};
this._onPlayerPauseListener=function(f){c._onPlayerPause.apply(c,arguments)
};
...
}},mediaPause:function(d,b,c){if(this._active){this._paused=d;
var a={globalDataType:"com.theplatform.pdk.data::MediaPause",clip:b,userInitiated:!!c};
if(d){this._controller.dispatchEvent("OnMediaPause",a)
...
this._pauseSetCallback(true,null,d.data.userInitiated)
...
this._pauseSetCallback(false,null,b.data.userInitiated)
...
if(this.controller.callFunction("getMuteState",[])){this.adManager.setVolume(0)
...
}},_mediaPauseSet:function(c,a,b){if(!this.adManager||!this.clip||c===this.paused){return
}this.paused=c;
if(c){this._stopProgressTimer();
this.adManager.setAdPlayState(c)
}else{this._startProgressTimer();
this.adManager.setAdPlayState(c)
}this._noopPlayer.mediaPause(c,this.clip,b)
...
$pdk.ns("$pdk.plugin.freewheel.slotmanager");
$pdk.plugin.freewheel.slotmanager.MultiClipSlotManager=$pdk.extend($pdk.plugin.freewheel.slotmanager.SlotManager,{impression:false,load:false,constructor:function(a,d,b,c){$pdk.plugin.freewheel.slotmanager.SlotManager.call(this,a,d);
...
this.adManager.addEventListener(tv.freewheel.SDK.EVENT_AD_COMPLETE,this.bind(this,this._onAdComplete));
this.adManager.addEventListener(tv.freewheel.SDK.ERROR_TIMEOUT,this.bind(this,this._onAdTimeout));
...
}},next:function(a){if(a&&this.playlist){if(a.baseClip.isAd&&a.streamType==="empty"){this.load=true;
this.clip=a;
this._checkStart()
}}else{if(!a||(a.baseClip.isAd&&a.streamType==="empty")){this.controller.nextClip()
...
}},skipCurrentAd:function(){if(this.slot){this.log("Skipping Current Ad due to "+(this.timeout/1000)+" second delay.",tpConsts.INFO);
this._endClip();
this._startTimeout();
this.slot.skipCurrentAd()
}},_onSlotStarted:function(a){this.log("Slot Started",tpConsts.INFO);
if(!isNaN(this.timeout)){this._startTimeout()
}},_onSlotEnded:function(b){if(this._isLinearSlot(b.slot)){this._stopProgressTimer();
this.log("Slot Ended",tpConsts.INFO);
clearTimeout(this._timeoutId);
if(!this.clipError&&this.playlist&&this.playlist.clips&&this.playlist.clips.length){for(var a=this.index+1;
a<this.playlist.clips.length;
a++){this.controller.nextClip()
}}}},_isLinearSlot:function(a){return(a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_PREROLL||a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_MIDROLL||a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_POSTROLL)
},_onAdImpression:function(b){this.log("Impression",tpConsts.INFO);
clearTimeout(this._timeoutId);
var c=b.slot||b.adInstance.getSlot();
...
this._checkStart()
}},_onAdTimeout:function(a){this._endClip()
},_onAdEvent:function(a){if(a.subType===tv.freewheel.SDK.EVENT_ERROR){if(this._isLinearSlot(a.slot)){this._onLinearSlotError(a)
}}else{if(a.subType===tv.freewheel.SDK.EVENT_AD_PAUSE){this._mediaPauseSet(true)
}}},_onLinearSlotError:function(a){switch(a.errorCode){case"_e_device-limit":this.controller.nextClip();
break;
case"_e_security":break;
default:this._endClip()
}this.clipError=true
},_onAdComplete:function(a){},_startTimeout:function(){var a=this;
this._timeoutId=setTimeout(function(){a.skipCurrentAd()
},this.timeout)
...
this._noopPlayer.mediaStarts(this.clip);
this._startProgressTimer()
},_endClip:function(){if(this.clip){if(!this._noopPlayer._active){this._noopPlayer.activate();
this._noopPlayer.mediaStarts(this.clip)
...
}if(c.vars.adTimeout){this.adTimeout=parseInt(c.vars.adTimeout,10)||this.adTimeout
}if(this.adTimeout<100){this.adTimeout=100
...
if(a){this.log("onOverlayAreaChanged() width:"+a.width+" height:"+a.height);
this.videoDisplayBase.style.width=a.width+"px";
this.videoDisplayBase.style.height=a.height+"px"
}},onMediaAreaChanged:function(){var a=this.controller.getMediaArea();
if(a){if(this.currentClip){var e=this.adManager.getSlotByCustomId(this.currentClip.baseClip.guid);
if(e&&typeof(e.getAdInstances)==="function"){var c=e.getAdInstances(false);
var d;
for(var b=0;
b<c.length;
b++){d=c[b].getRendererController().getRenderer();
if(d&&d.vpaidViewmode){d.vpaidViewmode=(this.controller.getFullScreenState()?"fullscreen":"normal")
}}}}if(this.adManager&&this.adManager.ready){this.adManager.setVideoDisplaySize(a.left,a.top,a.width,a.height)
...
},onLoadReleaseUrl:function(f){if(this.adManagerError){return
}this.log("onLoadReleaseUrl():"+this.releaseUrl);
this.isOnLoadReleaseUrlInvoked=true;
var c=f.data;
var b=this.getReleaseUrl(c.url);
if(b==this.releaseUrl){this.log("onLoadReleaseUrl invoked after onReleaseStart, do not send request");
this.isOnLoadReleaseUrlInvoked=false;
this.releaseUrl=null;
return
}this.releaseUrl=b;
...
var e=null;
if(this.parameters.customVideoAssetOverrideId){e=this.parameters.customVideoAssetOverrideId
}else{if(c&&this.parameters.customVideoAssetIdField){var a=this.parameters.customVideoAssetIdField.split(".");
for(field in c){if(field.indexOf("$">1)){if(field.split("$")[1]==a[0]){e=c[field];
if(a[1]&&typeof e==="object"){e=e[a[1]]
}}}}this.log("Trying to get VideoAssetCustomId from release['"+this.parameters.customVideoAssetIdField+"']: "+e)
}}if(!e&&this.testCustomValue){e=this.testCustomValue
}if(!e&&c&&c.id){e=c.id.substr(c.id.lastIndexOf("/")+1);
this.log("Use release.id for videoAssetCustomId:"+e)
}var d=null;
if(c&&this.parameters.keyValuesField){for(field in c){if(field.indexOf("$">1)){if(field.split("$")[1]==this.parameters.keyValuesField){d=c[field]
}}}this.log("Getting keyValues from Media ['"+this.parameters.keyValuesField+"']: "+d)
}this.buildCurrentVideoInfo(e,null,d);
if(!this.hasPrebuffered){this.submitRequest()
...
this.cleanup()
},onReleaseError:function(a){this.log("onReleaseError()");
this.cleanup()
},cleanup:function(){if(this.slotManager){this.slotManager.stop()
}this.resetDataForNewRelease()
...
a<b.length;
a++){var d=b[a];
if(this.isTemporalSlot(d)&&d.getAdCount()===0){this.log("adding temporal slot to cuePointManager: "+d.getCustomId());
this.cuePointManager.addTemporalSlot(d)
}else{switch(d.getTimePositionClass()){case tv.freewheel.SDK.TIME_POSITION_CLASS_PREROLL:if(d.getAdCount()&&d.getCustomId()!=="_fw_empty_preroll_slot"){this.log("adding preroll slot:"+d.getCustomId()+" to position:"+this.prerollSlots.length,tpConsts.DEBUG);
this.prerollSlots.push(d)
}break;
case tv.freewheel.SDK.TIME_POSITION_CLASS_POSTROLL:if(d.getAdCount()){this.log("adding postroll slot:"+d.getCustomId()+" to position:"+this.postrollSlots.length,tpConsts.DEBUG);
...
case tv.freewheel.SDK.TIME_POSITION_CLASS_OVERLAY:if(d.getAdCount()){this.log("adding overlay ads id:"+d.getCustomId()+" num ads:"+d.getAdCount(),tpConsts.DEBUG);
this.cuePointManager.addTemporalSlot(d)
}break
...
var c;
if(this.isFreeWheelClip(a)){c=this.adManager.getSlotByCustomId(a.baseClip.guid);
...
}}}else{if(a.baseClip&&!a.baseClip.isAd){c=this.getPlayableSlotAtTime(a.startTime/1000);
if(c){c.play()
}}}},onMediaStart:function(a){this.log("onMediaStart");
if(this.adManager&&a.data){if(a.data.baseClip.isAd===false){this.adManager.setVideoState(tv.freewheel.SDK.VIDEO_STATE_PLAYING)
}else{this.adManager.setVideoState(tv.freewheel.SDK.VIDEO_STATE_PAUSED)
}if(a.data.baseClip.bitrate>0){this.setDesiredBitrate(a.data.baseClip.bitrate)
}}},onMediaPlaying:function(a){if(!a.data.isAd){var b=a.data.currentTimeAggregate/1000;
this.adManager.setVideoAssetCurrentTimePosition(b);
var c=this.getPlayableSlotAtTime(b);
if(c&&!this.isPrerollSlot(c)){c.play()
}}},getPlayableSlotAtTime:function(a){if(this.adManager.ready){return this.cuePointManager.getPlayableSlot(a)
}},isPrerollSlot:function(a){return a&&a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_PREROLL
