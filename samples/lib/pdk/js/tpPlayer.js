...
if($pdk.bootloaderVersion!==null&&typeof($pdk.bootloaderVersion)==="object"){if("5"!=$pdk.bootloaderVersion.major||"7"!=$pdk.bootloaderVersion.minor||"12"!=$pdk.bootloaderVersion.revision){alert("Error: Bootloader Version and PDK-JS Version do not match.\n\nSomething is seriously wrong.")
...
this._controller.addEventListener("OnReleaseStart",function(b){a.handleReleaseStart(b)
});
this._controller.addEventListener("OnMediaLoadStart",function(b){a.handleMediaLoadStart(b)
});
...
}catch(c){}},handleReleaseStart:function(a){this._isPrerollTracking=true;
this.determineIfPlaylistHasSsa(a.data);
if(a.data.clips&&a.data.clips.length>0){this.gatherContentClipInfo(a.data.clips)
}},determineIfPlaylistHasSsa:function(c){var b=c.metaTags;
if(b){for(var a=0;
a<b.length;
a++){if(b[a].name==="manifestServiceUrl"){this._playlistHasSsa=true;
break
}}}},gatherContentClipInfo:function(a){this._contentClipIndexesByClipId={};
this._contentClipBoundaries=[];
var c=0;
for(var b=0;
b<a.length;
b++){var e=a[b];
if(!e.baseClip.isAd){this._contentClipIndexesByClipId[e.id]=c;
if(!this._contentClipIndexesByClipId.firstContentClipId){this._contentClipIndexesByClipId.firstContentClipId=e.id
}c++;
var d={startTime:e.startTime,endTime:e.endTime};
this._contentClipBoundaries.push(d)
}}this._contentClipIndexesByClipId.numberOfContentClips=c
},handleMediaLoadStart:function(b){this._clip=b.data;
this._isAd=b.data.baseClip.isAd;
this._trackingURLs=this.getTrackingURLs(this._clip);
this.handlePossibleBreakStartAndEndFromEmptyAdBreak();
var a=function(d,c){return d==c
};
if(a(this._clip.id,this._contentClipIndexesByClipId.firstContentClipId)){this._isPrerollTracking=false
...
}},isLastContentClip:function(a){return(a&&!a.baseClip.isAd&&this._contentClipIndexesByClipId[a.id]==this._contentClipIndexesByClipId.numberOfContentClips-1)
},fireEmptyAdBreakStartBreakEndBeacons:function(){for(var b=0;
b<this._trackingURLs.length;
b++){var a=this._trackingURLs[b];
if(!a.hasFired&&a.URL&&a.URL.length>0&&this.matchUrlToClip(a)){if(a.triggerType==$pdk.managers.TrackingUrl.TRIGGER_TYPE_EVENT){if(a.triggerValue===$pdk.managers.TrackingUrl.EVENT_START){this.removeEmptyAdBreakDelimiter(a);
this.trackProgress(a)
}else{if(a.triggerValue===$pdk.managers.TrackingUrl.EVENT_COMPLETE&&this._handleMediaCompleteFired){this.removeEmptyAdBreakDelimiter(a);
this.trackProgress(a)
}}}}}},matchUrlToClip:function(a){var c=this._contentClipIndexesByClipId[this._clip.id];
var d=this.CHAPTER_INDEX_DELIMITER+c+this.CHAPTER_INDEX_DELIMITER;
var g=a.URL.indexOf(d);
var f=g>-1;
var e=a.URL.indexOf(this.CHAPTER_INDEX_DELIMITER+"POSTROLL"+this.CHAPTER_INDEX_DELIMITER);
var b=e>-1;
if(b){a.triggerValue=$pdk.managers.TrackingUrl.EVENT_COMPLETE
}else{a.triggerValue=$pdk.managers.TrackingUrl.EVENT_START
}return f
},removeEmptyAdBreakDelimiter:function(a){var b=a.URL.indexOf(this.CHAPTER_INDEX_DELIMITER);
a.URL=a.URL.substr(0,b)
},isVideoViewTracking:function(a){var d=this.getQueryString("cn",a);
if(!d||d!="videoView"){return false
}var c=this.getQueryString("init",a);
if(!c||c!="1"){return false
}var b=(a.indexOf("://")>-1)?a.split("/")[2]:a.split("/")[0];
b=b.split(":")[0];
return b.indexOf("fwmrm.net")>-1
},getQueryString:function(d,b){var c=new RegExp("[?&]"+d+"=([^&#]*)","i");
var a=c.exec(b);
return a?a[1]:null
},replaceQueryParameter:function(e,c,g,d){var a=new RegExp("[?]"+e+"=([^&#]*)","i");
var f=new RegExp("[?&]"+e+"=([^&#]*)","i");
var b=c.match(a)?"?":"&";
return c.replace(f,b+g+"="+d)
},startVideoViewTracking:function(a){if(!this._onVideoViewMediaPlayingListener){var b=this;
this._videoViewTime={url:this.replaceQueryParameter("init",a.URL,"init","0"),cumulativeSeconds:0,lastTime:0,lastSent:this._videoViewCumulativeTimes[0],lastCumulativeSent:0,nextToSend:this._videoViewCumulativeTimes[1]};
this._onVideoViewSeekListener=function(c){b.handleVideoViewSeek(c)
};
this._controller.addEventListener("OnMediaSeek",this._onVideoViewSeekListener);
this._onVideoViewMediaPlayingListener=function(c){b.handleVideoViewMediaPlaying(c)
};
...
this._videoViewTime={}
}},handleVideoViewSeek:function(a){if(!this._isAd){this._videoViewTime.lastTime=0
}},handleVideoViewMediaPlaying:function(b){if(this._isAd){return
}if(this._videoViewTime.lastTime&&this.isValidMediaPlaying(b.data.currentTime,this._videoViewTime.lastTime,b.data.mediaTime)){this._videoViewTime.cumulativeSeconds+=(b.data.currentTime-this._videoViewTime.lastTime)/1000;
...
var a=this._videoViewCumulativeTimes.indexOf(this._videoViewTime.lastSent);
this._videoViewTime.nextToSend=a<this._videoViewCumulativeTimes.length-1?this._videoViewCumulativeTimes[a+1]:this._videoViewCumulativeTimes[a]
}}this._videoViewTime.lastTime=b.data.currentTime
},isValidMediaPlaying:function(b,d,a){var c=false;
if(!this.isFromScrubberDrag(b,d)){if(($pdk.isSafari||$pdk.isIOS)&&this._playlistHasSsa){c=this.isWithinContentClipBoundaries(a)
}else{c=true
}}return c
},isFromScrubberDrag:function(b,c){var a=b-c;
return a<=0||a>750
},isWithinContentClipBoundaries:function(b){for(var a=0;
a<=this._contentClipBoundaries.length;
a++){if(b>=this._contentClipBoundaries[a].startTime&&b<=this._contentClipBoundaries[a].endTime){return true
}}return false
},sendNextVideoView:function(a){this.sendUrl(this._videoViewTime.url+"&ct="+a)
},handleMediaStart:function(b){this._viewabilityStartTime=0;
...
var a=this;
this._onMediaPlayingListener=function(c){a.handleMediaPlaying(c)
};
this._controller.addEventListener("OnMediaPlaying",this._onMediaPlayingListener)
}}if(this._videoViewTime&&this._videoViewTime.lastTime){this._videoViewTime.lastTime=0
...
}},handleReleaseEnd:function(a){this._handleMediaCompleteFired=true;
this.handlePossibleBreakStartAndEndFromEmptyAdBreak();
this._handleMediaCompleteFired=false;
this.resetTrackingURLs();
this.endVideoViewTracking()
...
},getTrackingURLs:function(e){var b=e.baseClip.isAd;
var a=b?e.baseClip.releaseID:e.id;
var d=this;
var c=function(){return b&&d._trackingClipMap[a].baseClip===e.baseClip
};
var f=function(){return !b&&d._trackingClipMap[a]
};
if(this._trackingClipMap[a]&&(c()||f())){tpDebug("Use existing Clip's trackingURLs","TrackingUrlMgr",tpConsts.INFO);
return this._trackingURLMap[a]
}this._trackingClipMap[a]=e;
this._trackingURLMap[a]=e.baseClip.trackingURLs;
return this._trackingURLMap[a]
},resetTrackingURLs:function(){this._trackingClipMap={};
this._trackingURLMap={}
},doTracking:function(f){if(!this._trackingURLs){return
}var d;
var c;
var e;
if(!f){f={};
f.percentComplete=100;
f.currentTime=1000000;
e=5
}else{e=100000/(!this._isAd&&f.durationAggregate?f.durationAggregate:f.duration)
}c=(!this._isAd&&f.currentTimeAggregate)?f.currentTimeAggregate:f.currentTime;
d=(!this._isAd&&f.percentCompleteAggregate)?f.percentCompleteAggregate:f.percentComplete;
var b;
for(var a=0;
a<this._trackingURLs.length;
++a){b=this._trackingURLs[a];
if(!b.hasFired&&b.URL&&b.URL.length>0&&!this._eventTracking.trackingUrlIsTaggedForSpecificClip(b)){if((b.triggerType==$pdk.managers.TrackingUrl.TRIGGER_TYPE_PERCENTAGE)&&(d>=b.triggerValue)&&(d<(b.triggerValue+5*e))){this.trackProgress(b)
}else{if(b.triggerType==$pdk.managers.TrackingUrl.TRIGGER_TYPE_MILLISECONDS&&c>=b.triggerValue&&c<(b.triggerValue+5000)){this.trackProgress(b)
}else{if(b.triggerType==$pdk.managers.TrackingUrl.TRIGGER_TYPE_EVENT&&b.triggerValue==$pdk.managers.TrackingUrl.EVENT_COMPLETE&&d==100){this.trackBreakEnd(b)
}}}}}},trackBreakEnd:function(a){this.trackProgress(a);
if(this._isPrerollTracking&&this.isVideoViewTracking(a.URL)){this.startVideoViewTracking(a)
...
if(!a||this.urlWasNotDefined(a)){return
...
c.src=a+"&"
},urlWasNotDefined:function(a){return a==="undefined"
...
}},doSetup:function(a){if(this.trackingUrlIsTaggedForSpecificClip(a)){return
}var b=this;
...
break
}},trackingUrlIsTaggedForSpecificClip:function(a){return typeof a.URL==="string"&&a.URL.match(/_\|\|_\d+_\|\|_/)!==null
...
if(tpIsAndroid()||this._pbm.video.webkitDisplayingFullscreen||document.webkitFullscreenElement||document.mozFullScreenElement||document.msFullscreenElement||this._isFullWindow){a=true
}else{a=false
...
a.registerFunction("setFairPlayCertUrl",this,this.setFairPlayCertUrl);
...
},setFairPlayCertUrl:function(a){this.cd.setFairPlayCertUrl(a)
},setClip:function(a){this.currentClip=a;
this.cd.setMediaData(a.baseClip.security);
this.cd.flush()
},preloadClip:function(a){return this.cd.preload(a)
...
},onMediaLoadStart:function(d){var a=d.data;
var c=a.baseClip.security;
var b=(a.isAd===false||a.baseClip.isAd===false);
if(b||c){this.currentClip=a;
this.cd.setMediaData(c)
...
},_getFairPlayCertUrl:function(b){var a;
for(var c in b){if(c.indexOf("fairplaycerturl")===0){a=b[c]
}}return a
...
this.wrapper=e;
this.mpx=d;
var a={widevine:"//widevine.entitlement."+d+"/wv/web/ModularDrm?form=json&schema=1.0&token=<token>&account=http://access.auth."+d+"/data/Account/<account>",fairplay:"//fairplay.entitlement."+d+"/fpls/web/FairPlay?form=json&schema=1.0&token=<token>&account=http://access.auth."+d+"/data/Account/<account>",playready:"//playready.entitlement.next."+d+"/playready/rightsmanager.asmx?form=json&schema=1.0&auth=<token>&account=http://access.auth."+d+"/data/Account/<account>&releasePid=<releasePid>"};
...
}}},preload:function(a){if(this.cd&&this.cd.token){this.setMediaData(a.baseClip.security);
this.flush();
return true
}return false
},setFairPlayCertUrl:function(a){if(a!=undefined){this.cd.fairPlayCertUrl=a
}},setToken:function(a){this.cd.token=a
},setMediaData:function(c,d,b,e){if(c){var a=c;
if(a.toLowerCase()==="commonencryption"){a=this._handleCommonEncryption(a)
}this.cd.security=a
}if(d){this.cd.releasePid=d
}if(b){this.cd.mediaId=b
}if(e){this.cd.accountId=e
}},_handleCommonEncryption:function(a){if($pdk.isChrome||$pdk.isFirefox){return"widevine"
...
if(!b.baseClip.isAd&&!b.baseClip.noSkip){this.controlsManager.setChapterPoints(!this.currentChapters?[]:this.currentChapters)
...
this.plugins=new Array();
e.subtitlesLoader=new $pdk.queue.deferred.loader.Subtitles(e.controller,g,document.getElementById(e.controller.id+".subtitles"),f,6,i,b,true,e.controller.getProperty("enableDynamicSubtitleFonts")||"true");
...
},preloadContent:function(){var a=this.getFirstContentClip(this.playlist);
var c=this.getFirstClipWithMatchingURL(a.URL,this.playlist);
if(c&&!(tpIsIOS()||tpIsAndroid())){var d=c.startTime+c.offset;
var b=this.player.createMediaFileFromClip(c);
if(c.baseClip.security){if(this._clipIsKnownSecurityEncoding(c)&&this.challengeData&&this.challengeData.preloadClip(a)){this.videoEngine.preload(b,d)
}}else{this.videoEngine.preload(b,d)
}}},_clipIsKnownSecurityEncoding:function(b){var a=b.baseClip.security.toLowerCase();
return a==="playready"||a==="widevine"||a==="fairplay"||a==="commonencryption"
},getFirstContentClip:function(b){var a=function(c){return !c.baseClip.isAd
};
return this.findFirstInPlaylistByCondition(a,b)
},getFirstClipWithMatchingURL:function(b,c){var a=function(d){return d.URL===b
};
return this.findFirstInPlaylistByCondition(a,c)
},findFirstInPlaylistByCondition:function(a,d){if(d&&d.clips){for(var b=0;
b<d.clips.length;
b++){var c=d.clips[b];
if(a(c)){return c
}}}return null
...
},dispatchReleaseError:function(b,a){if(b){this.error=b;
...
if(b){this.doPauseVideo(a)
}else{this.doUnPauseVideo(a)
}}},doPauseVideo:function(a){if(this.currentClip&&this.currentClip.streamType==="empty"){if(!this.isNoopClip(this.currentClip)){this.controller.getVideoProxy().pause()
}this.controller.dispatchEvent("OnPlayerPause",{userInitiated:a})
}else{if(this.videoEngine.isPaused()==false){this.videoEngine.pause();
this.controller.dispatchEvent("OnPlayerPause",{userInitiated:a})
}}},doUnPauseVideo:function(a){this.play();
this.controller.dispatchEvent("OnPlayerUnPause",{userInitiated:a});
...
},_browserSupportsVTT:function(){return $pdk._browserCheck([{browser:"chrome",version:31,device:false},{browser:"chrome",version:39,os:"ios"},{browser:"safari",version:7.1},{browser:"firefox",version:33},{browser:"ipad",osversion:8},{browser:"iphone",osversion:8},{browser:"ipod",osversion:8},{browser:"chrome",version:39,os:"android"},{browser:"android",version:4.4}])
...
}},blockedByDrm:function(e){var c=false;
if(e){var b=this.getFirstContentClip(e);
var d=b.baseClip.security;
var a={};
if(d){a.security=[d.toLowerCase()]
}c=this.player.videoEngine.blockedByDrm(a)
}return c
},protectedByDrm:function(c){var a=this.getFirstContentClip(c);
var b=a.baseClip.security.toLowerCase();
return b==="widevine"||b==="fairplay"||b==="playready"||b==="commonencryption"
},contentIsHttps:function(b){var a=this.getFirstContentClip(b);
return a&&a.URL.indexOf("https")===0
},_isWidevineInChrome:function(c){var a=this.getFirstContentClip(c);
var b=a.baseClip.security.toLowerCase();
return $pdk.isChrome&&(b==="widevine"||b==="commonencryption")
},doPlayPlaylist:function(e){if(!e||e.isError){this.dispatchReleaseError({title:e.baseClips[0].title,description:e.baseClips[0].description,exception:e.baseClips[0].contentCustomData.exception,responseCode:e.baseClips[0].contentCustomData.responseCode,alternateSrc:e.baseClips[0].URL},this.releaseUrl);
return
}if(this.protectedByDrm(e)){var a=typeof navigator.requestMediaKeySystemAccess==="function";
if(this._isWidevineInChrome(e)&&!a){var c=this.contentIsHttps(e);
if(c){tpDebug("Chrome requires SSL site delivery for Widevine support.","ChallengeDataWrapper",this.controller.id,tpConsts.ERROR);
...
return
}}}if(this.blockedByDrm(e)){this.dispatchReleaseError({title:"Security Error",description:"This protected content cannot be played on the current system."},this.releaseUrl);
return
}var d=e.metaTags;
if(d){var b=0;
for(;
b<d.length;
b++){if(d[b].name==="manifestServiceUrl"){this.playlistHasSSA=true
}}}if($pdk.isAndroid&&!$pdk.isAndroid44plus&&!this.playlistHasSSA){this.stripM3U(e)
...
if(this.currentRelease){this.mergeReleaseWithPlaylist(this.currentRelease,e);
e.release=this.currentRelease;
e.release.mediaPID=e.mediaPID
}else{e.release=this.getReleaseFromPlaylist(e)
}this.controller.dispatchEvent("OnReleaseStart",e);
this.releaseProcess.setPlaylist(e)
...
}var a=!!this.releaseProcess;
this.releaseProcess=undefined;
this.currentClip=null;
if(a){this.controller.dispatchEvent("OnReleaseEnd",this.playlist)
}},clearCurrentRelease:function(){this.releaseCleared=true;
...
if(this.releaseProcess.seekToPosition(d.end.currentTimeAggregate)){this.videoEngine.preMediaEnd();
...
this._doGetNextClip()
}else{if(!this.releaseCleared){this.advanceToNextRelease(false,false)
...
if(a.baseClip.security&&a.baseClip.security.toLowerCase()==="playready"&&(!$pdk.isEdge)){this.challengeData.setClip(a)
...
var f=this;
var h=function(j){if(f.videoEngine.isPaused()&&(!f._hideNativeControls)){if(!tpIsIOS()&&!tpIsAndroid()){f.controller.pause(false)
}return true
...
if(tpIsIOS()){anchor.ontouchend=h
}else{anchor.onclick=h
...
if(this.videoEngine.getAudioTrackByIndex(a)){this.currentPreferredLanguage=this.videoEngine.getAudioTrackByIndex(a).language
...
this.currentPreferredLanguage=a
...
this.video=null
}},videoClicked:function(d){if(this.currentClip){if(!tpIsIOS()&&!tpIsAndroid()&&!this.isPausedWithClickThrough(this.currentClip)&&this.pauseOnClick===true){this.controller.pause(!this.getPauseState(),true)
...
},isPausedWithClickThrough:function(b){var c=this.clipHasClickThroughUrl(b);
var a=this.getPauseState();
return c&&a
},clipHasClickThroughUrl:function(a){return a.baseClip.moreInfo&&a.baseClip.moreInfo.href
...
if(tpIsIOS()){this.videoEngine.addEventListener("MEDIA_SEEKING",this.listeners.seeking=function(b){tpDebug("Video tag dispatched "+b.type);
a.onSeeking(b)
},false)
...
this.videoEngine.addEventListener("INITIAL_AUDIO_TRACK",this.listeners.initialAudioTrackSwitched=function(b){a.onInitialAudioTrackSwitched(b)
},false);
...
this.videoEngine.addEventListener("MEDIA_BUFFER_START",this.listeners.mediaBufferStart=function(b){a.onMediaBufferStart(b)
},false);
this.videoEngine.addEventListener("MEDIA_BUFFER_COMPLETE",this.listeners.mediaBufferComplete=function(b){a.onMediaBufferComplete(b)
},false);
this.videoEngine.addEventListener("RENDITION_SWITCHED",this.listeners.renditionSwitched=function(b){a.onRenditionSwitched(b)
},false);
this.videoEngine.addEventListener("MEDIA_CUE_POINT",this.listeners.mediaCuePoint=function(b){a.onMediaCuePoint(b)
},false)
...
if(!this.applyVideoEngineSettingsInterval){this.applyVideoEngineSettingsInterval=setInterval(function(){d.applyVideoEngineSettings();
clearInterval(d.applyVideoEngineSettingsInterval)
},50)
...
this.videoEngine.setAudioTrackByLanguage(this.currentPreferredLanguage)
}if(this.currentClip.baseClip.isAd&&this.videoEngine.isPaused()){this.videoEngine.play()
}if(this.clipHasClickThroughUrl(this.currentClip)){this.videoClickAnchor.style.cursor="pointer"
...
},onMediaBufferStart:function(a){this.controller.dispatchEvent("OnMediaBufferStart",this.currentClip)
},onMediaBufferComplete:function(a){this.controller.dispatchEvent("OnMediaBufferComplete",this.currentClip)
},onRenditionSwitched:function(a){this.controller.dispatchEvent("OnRenditionSwitched",a.event)
},onMediaCuePoint:function(a){this.controller.dispatchEvent("OnMediaCuePoint",a.event)
...
}},onSeeking:function(a){if(this.playlistHasSSA&&tpIsIOS()&&this.videoEngine.isFullScreen()&&!this.currentClip.baseClip.noSkip){clearInterval(this.timeUpdateTimer)
...
},onInitialAudioTrackSwitched:function(a){if(this._dispatchInitialAudioTrack===true){this.onAudioTrackSwitched(a);
...
if(this.timeUpdateTimer){clearInterval(this.timeUpdateTimer)
}},onTimeUpdate:function(g){if(this.currentClip===undefined||this.currentClip===null){return
...
if(this._shouldEndClip(d,c)){this.currentVideoTime=this.videoEngine.getCurrentTime();
if(this.currentClip.baseClip.isAd){this.currentClip.hasPlayed=true
}this.playbackManager.updateClip(this.currentClip);
tpDebug("Calling onEnded() at "+this.videoEngine.getCurrentTime());
this.onEnded()
}this.previousTime=d
}}}}}},_shouldEndClip:function(c,b){var a=this.playlistHasSSA&&this.usesNativeControls&&c>this.currentClip.endTime+500;
if(a){return false
}var d=(this.currentClip&&this.currentClip.endTime&&this.currentClip.endTime>0&&(!this.playlistHasSSA?(Math.abs(c-this.currentClip.endTime)<300||this.isSeeking&&c>this.currentClip.endTime):c>this.currentClip.endTime)&&Math.abs(c-b)>1000);
return d
...
},applyVideoEngineSettings:function(){var e=["limitRenditionsByPlayerSize","maxBufferDuration","startupRendition","withCredentials"];
var c=[];
for(var a=0;
a<e.length;
a++){var b=e[a];
var d=this.controller.getProperty(b);
if(d){c.push(b+"="+d)
}}if(c.length>0){this.videoEngine.applySettings(c)
...
this.isIOS_compatible=$pdk._browserCheck([{browser:"ipad",osversion:8},{browser:"iphone",osversion:8},{browser:"ipod",osversion:8},{browser:"chrome",version:39,os:"ios"},]);
if(!this.isIOS_compatible&&a){b.addEventListener("OnMediaEnd",function(f){c.nativeControlsOnMediaEnd(f)
});
b.addEventListener("OnMediaStart",function(f){c.nativeControlsOnMediaStart(f)
});
b.addEventListener("OnReleaseStart",function(f){c.nativeControlsOnReleaseStart(f)
})
...
if(this.isIOS_compatible){b.addEventListener("OnReleaseStart",function(f){c.nativeControlsOnReleaseStart(f)
});
b.addEventListener("OnMediaLoadStart",function(f){c.iosOnMediaLoadStart(f)
});
b.addEventListener("OnMediaEnd",function(f){c.nativeControlsOnMediaEnd(f)
});
...
}},nativeControlsOnMediaStart:function(b){this.log("iosOnMediaStart");
var a=b.data;
...
}}},nativeControlsOnMediaEnd:function(b){this.log("iosOnMediaEnd");
var a=b.data;
...
},nativeControlsOnReleaseStart:function(d){this.log("iosOnReleaseStart");
